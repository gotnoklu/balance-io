...
import { durations, taskTypes } from '../../constants/app'
import { parseTimeStringFormat } from '../../utilities/global'

const TaskDialog = ( { isOpen, onClose, title, editDetails, onEdit, onSave } ) => {
	...
	const handleSaveDetails = () => {
		if ( !name.length ) {
			setNameFieldHelperText( 'You need to supply a name for the activity.' )
		...
		const createTaskTypeDetails = ( type, details ) => {
			...
			const baseTemplate = { id, name: name.trim(), description, completed, reminder: false }
	...
	return (
		<Dialog title={title} isOpen={isOpen} onClose={onClose} actions={dialogActions}>
