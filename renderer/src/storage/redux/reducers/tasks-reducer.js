...
const { SET_TASKS_STORE, DELETE_ALL_TASKS, DELETE_MULTIPLE_TASKS, DELETE_ONE_TASK, SAVE_TASK } =
	tasksReducerActions
...
const tasksReducer = ( state = defaultTaskStore, action ) => {
	switch ( action.type ) {
		case HYDRATE: {
			let updatedState = { ...action.payload.tasks }
			return compareStates( defaultTaskStore, state, updatedState )
		}
